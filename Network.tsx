...
export default function NetworkPage() {
  ...
  useEffect(() => {
    const fetchIP = async () => {
      // Try multiple providers in case one is blocked or fails
      const providers = [
        { url: 'https://ipapi.co/json/', map: (d: any) => ({
          ip: d.ip, city: d.city, org: d.org, country_name: d.country_name, timezone: d.timezone
        })},
        { url: 'https://api.ipify.org?format=json', map: (d: any) => ({
          ip: d.ip, city: '-', org: '-', country_name: '-', timezone: '-'
        })},
        { url: 'https://ip-api.com/json', map: (d: any) => ({
          ip: d.query, city: d.city, org: d.as, country_name: d.country, timezone: d.timezone
        })}
      ];
      ...
      for (const provider of providers) {
        try {
          ...
          if (res.ok) {
            ...
            setIpData(provider.map(data));
            return;
          }
        } catch (e) {
          console.error(`Failed to fetch IP from ${provider.url}`);
        }
      }
      setIpData({ ip: 'Unavailable', city: 'Unknown', org: 'Unknown' });
    };
